...
void state::unmake_move(int move) {
	...
	if (((move >> 18) & 3) == 3) {
		...
		if (!to_move) {
			whitepawn_row_sum += 6;
			if (white_pawn_counts[col_init] >= 2) doubled_white -= white_pawn_counts[col_init];
			...
			blackpawn_row_sum += 6;
			if (black_pawn_counts[col_init] >= 2) doubled_black -= black_pawn_counts[col_init];
		...
		if (piece == WP) {
			if (!to_move) {
				...
				if (white_pawn_counts[col_init] >= 2) doubled_white -= white_pawn_counts[col_init];
				if (white_pawn_counts[col_final] >= 2) doubled_white -= white_pawn_counts[col_final];
				white_pawn_counts[col_init]++;
				...
				for (int i = 0; i < whitepawns.size(); i++) {
					if (whitepawns[i].first == row_final && whitepawns[i].second == col_final) {
						whitepawns[i].first = row_init; whitepawns[i].second = col_init;
						break;
					}
				}
			}
				...
				if (black_pawn_counts[col_init] >= 2) doubled_black -= black_pawn_counts[col_init];
				if (black_pawn_counts[col_final] >= 2) doubled_black -= black_pawn_counts[col_final];
				black_pawn_counts[col_init]++;
				...
				for (int i = 0; i < blackpawns.size(); i++) {
					if (blackpawns[i].first == row_final && blackpawns[i].second == col_final) {
						blackpawns[i].first = row_init; blackpawns[i].second = col_init;
						break;
					}
				}
			}
			...
			if (!to_move) {
				...
				for (int i = 0; i < whiteknights.size(); i++) {
					if (whiteknights[i].first == row_final && whiteknights[i].second == col_final) {
						whiteknights[i].first = row_init; whiteknights[i].second = col_init;
						break;
					}
				}
			}
				...
				for (int i = 0; i < blackknights.size(); i++) {
					if (blackknights[i].first == row_final && blackknights[i].second == col_final) {
						blackknights[i].first = row_init; blackknights[i].second = col_init;
						break;
					}
				}
			}
			...
			if (!to_move) {
				...
				for (int i = 0; i < whitebishops.size(); i++) {
					if (whitebishops[i].first == row_final && whitebishops[i].second == col_final) {
						whitebishops[i].first = row_init; whitebishops[i].second = col_init;
						break;
					}
				}
			}
				...
				for (int i = 0; i < blackbishops.size(); i++) {
					if (blackbishops[i].first == row_final && blackbishops[i].second == col_final) {
						blackbishops[i].first = row_init; blackbishops[i].second = col_init;
						break;
					}
				}
			}
			...
			if (!to_move) {
				for (int i = 0; i < whiterooks.size(); i++) {
					if (whiterooks[i].first == row_final && whiterooks[i].second == col_final) {
						whiterooks[i].first = row_init; whiterooks[i].second = col_init;
						break;
					}
				}
			}
				...
				for (int i = 0; i < blackrooks.size(); i++) {
					if (blackrooks[i].first == row_final && blackrooks[i].second == col_final) {
						blackrooks[i].first = row_init; blackrooks[i].second = col_init;
						break;
					}
				}
			}
			...
			if (!to_move) {
				for (int i = 0; i < whitequeens.size(); i++) {
					if (whitequeens[i].first == row_final && whitequeens[i].second == col_final) {
						whitequeens[i].first = row_init; whitequeens[i].second = col_init;
						break;
					}
				}
			}
				...
				for (int i = 0; i < blackqueens.size(); i++) {
					if (blackqueens[i].first == row_final && blackqueens[i].second == col_final) {
						blackqueens[i].first = row_init; blackqueens[i].second = col_init;
						break;
					}
				}
			}
