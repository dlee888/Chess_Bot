...
void analyse_game(int depth) {
	...
	for (int i : game) {
		best_moves.push_back(find_best_move(depth, -DINF, DINF));
...
void get_best_move() {
	...
	while ((double)time_taken / CLOCKS_PER_SEC * curr_state.list_moves().size() < 2 * tlim) {
		...
		if (abs(best_move.first - 1000.0) < 1) break;
		if (abs(best_move.first + 1000.0) < 1) break;
		nodes = 0;
		curr_depth++;
	}
...
void get_best_move2() {
	...
	auto best_move = find_best_move(depth, -DINF, DINF);
	std::cout << "Best move is " << curr_state.move_to_string(best_move.second) << ", EVAL = " << best_move.first << std::endl;
}
