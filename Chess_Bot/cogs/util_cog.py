...
class AcceptDrawButton(discord.ui.Button):
    def __init__(self, client, game):
        super().__init__(
            label="Accept draw offer", style=discord.ButtonStyle.primary, emoji="🤝"
        )
        self.client = client
        self.game = game
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if (interaction.user.id == self.game.white and not self.game.black_draw) or (
            interaction.user.id == self.game.black and not self.game.white_draw
        ):
            await interaction.response.send_message(
                "Your opponent has not offered a draw.", ephemeral=True
            )
            return
        ...
        if interaction.user.id == self.game.white:
            await interaction.message.reply(
                f"Draw accepted. Your new rating is {round(await data.data_manager.get_rating(interaction.user.id), 3)} ({round(white_delta, 3)})"
            )
            file, embed, _ = await util2.make_embed(
                self.game.black,
                title="Your game has ended",
                description=f"{interaction.user} accepted your draw offer.\nYour new rating is {round(await data.data_manager.get_rating(self.game.black), 3)} ({round(black_delta, 3)})",
            )
            await util2.send_notif(self.game.black, file=file, embed=embed)
        else:
            await interaction.message.reply(
                f"Draw accepted. Your new rating is {round(await data.data_manager.get_rating(interaction.user.id), 3)} ({round(black_delta, 3)})"
            )
            file, embed, _ = await util2.make_embed(
                self.game.white,
                title="Your game has ended",
        ...
        await data.data_manager.delete_game(interaction.user.id, 69)
...
class ResignButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if interaction.user.id == self.game.white:
            await interaction.message.reply(
                f"Game resigned. Your new rating is {round(await data.data_manager.get_rating(interaction.user.id), 3)} ({round(white_delta, 3)})"
            )
            file, embed, _ = await util2.make_embed(
                self.game.black,
                title="Your game has ended",
                description=f"It was in this position that {interaction.user} resigned the game.\nYour new rating is {round(await data.data_manager.get_rating(self.game.black), 3)} ({round(black_delta, 3)})",
            )
            await util2.send_notif(self.game.black, file=file, embed=embed)
        else:
            await interaction.message.reply(
                f"Game resigned. Your new rating is {round(await data.data_manager.get_rating(interaction.user.id), 3)} ({round(black_delta, 3)})"
            )
            file, embed, _ = await util2.make_embed(
                self.game.white,
                title="Your game has ended",
        ...
        await data.data_manager.delete_game(
            interaction.user.id,
            chess.WHITE if interaction.user.id == self.game.black else chess.BLACK,
        )
...
class Util(commands.Cog):
    ...
    async def make_embed(self, person, *, title="", description="", flip=False):
        embed = discord.Embed(color=0x5EF29C, title=title, description=description)
        ...
        if person == game.white and game.black_draw:
            ...
            return (
                discord.File(path, filename="board.png"),
                embed,
                GameViewDraw(self.client, game),
            )
    ...
    async def send_notif(self, person, text="", **kwargs):
        ...
        if channel is not None:
            ...
            if is_default and random.randint(1, 3) == 1:
                text = (
                    "Tip: change your default notification channel by using the `$notif set` command!.\n"
                    + text
                )
